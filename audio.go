	...
	for {
		...
		if flags&wca.AUDCLNT_BUFFERFLAGS_SILENT == 0 {
			...
			*/
			...
			// If channel is getting full (>5 packets), drop oldest to prevent accumulation
			// Increased from 3 to 5 for smoother audio on slow networks
			for len(audioBroadcast) > 5 {
				select {
